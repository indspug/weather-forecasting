...
"""
  LSTMを用いた天気予測
    - 過去24時間の気象データから、4時間後の水戸の天気を予測する
"""
...
LENGTH_OF_SHIFT = 4		# 4時間後のデータを予測する
NUMBER_OF_INPUT_NODES = 12	# 入力データ数:(湿度,気圧,天気)x(水戸,東京,静岡,大阪)
...
NUMBER_OF_EPOCHS = 1		# 1回の学習のエポック数
NUMBER_OF_TRAINING = 300	# 学習回数
OUTPUT_CYCLE = 10		# 学習経過出力周期
RESULT_FILE_WHOLE = './result/rnn_result_190812_03_whole.csv'
RESULT_FILE_NAME  = './result/rnn_result_190812_03'
...
def output_whole_result_header():
	...
	fo.write('##################################################\n')
	fo.write('入力データ = (湿度,気圧,天気)x(水戸,東京,静岡,大阪)\n')
...
def output_result(input, target, predicted, number):
	...
	fo = open(filename, 'w')
	fo.write('水戸,,,静岡,,,東京,,,大阪,,,水戸(正解),,,水戸(予測),,,,\n')
	fo.write('湿度,気圧,天気,湿度,気圧,天気,湿度,気圧,天気,湿度,気圧,天気,晴れ,曇り,雨,晴れ,曇り,雨,正解/不正解\n')
	...
	for i in range(data_len):
		...
		if i < (LENGTH_OF_SEQUENCES+LENGTH_OF_SHIFT):
			...
			fo.write('%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%.2f,%.2f,%.2f\n' %
			...
			#print("%.2f,%.2f,%.2f" % (target, predict_i, correct) )
			#print("%d,%d,%d" % (correct_i, predict_i, correct) )
			
			fo.write('%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%d\n' % 
				(input_i[0], input_i[1], input_i[2], input_i[3],
				 input_i[4], input_i[5], input_i[6], input_i[7],
				 input_i[8], input_i[9], input_i[10], input_i[11],
				 target_i[0], target_i[1], target_i[2],
...
if __name__ == '__main__':
	...
	train_input_raw2, train_target_raw2 = load_data('./train/tokyo',    '東京')
	train_input_raw3, train_target_raw3 = load_data('./train/shizuoka', '静岡')
	train_input_raw4, train_target_raw4 = load_data('./train/osaka',    '大阪')
	train_input_raw = numpy.hstack(
				[train_input_raw1, train_input_raw2, 
				 train_input_raw3, train_input_raw4] )
	train_target_raw = train_target_raw1
	
	# テスト用データ取得
	test_input_raw1, test_target_raw1 = load_data('./test/mito',     '水戸')
	test_input_raw2, test_target_raw2 = load_data('./test/tokyo',    '東京')
	test_input_raw3, test_target_raw3 = load_data('./test/shizuoka', '静岡')
	test_input_raw4, test_target_raw4 = load_data('./test/osaka',    '大阪')
	test_input_raw = numpy.hstack(
				[test_input_raw1, test_input_raw2, 
				 test_input_raw3, test_input_raw4] )

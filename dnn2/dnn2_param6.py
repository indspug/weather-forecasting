...
# 分類用のパラメータ
##############################
NUMBER_OF_INPUT_NODES = 24	# 入力データ数:(水戸,前橋,東京,静岡,大阪,秩父,河口湖,新潟,宇都宮)x(気温,降水量,湿度,気圧)
...
RESULT_FILE_NAME  = './result5/result_190917_dnn2_06'
...
RESULT_FILE_WHOLE = './result5/result_190917_dnn2_06.csv'
SAVE_CYCLE = 1000		# 保存周期(N回学習につき1回保存)
MODEL_DIR  = 'ckpt'		# モデル保存先ディレクトリ
MODEL_NAME = 'model_190917_dnn2_06'	# 保存するモデルのファイル名
NUMBER_OF_POINTS = 6		# 入力データの地点数
...
def get_data():
	...
	print(test_input_raw.shape)
    	print(test_target_raw.shape)
	
	print(train_input_raw)
    	print(train_target_raw)
	print(test_input_raw)
    	print(test_target_raw)
...
def output_whole_result_header():
	...
	fo.write('##################################################\n')
	fo.write('入力データ = (水戸,前橋,東京,静岡,大阪,秩父)x(気温,降水量,湿度,気圧)\n')

...
MODEL_NAME = 'model_02'	# 保存するモデルのファイル名
NUM_EPOCH = 10		# 何回繰り返し学習させるか
NUM_TRAING = 1000	# 学習回数(NUM_TRAING * NUM_EPOCH)
SAVE_CYCLE = 100	# 保存周期(N回学習につき1回保存)
RESULT_FILE = 'result_190815_simple_dnn_02.csv'	# 学習経過保存ファイル
...
def extract_learning_data(dirpath):
	...
	for csv_path in csv_paths:
		...
		# 入力データ取得：気温,降水量,相対湿度,海面気圧,風速,風向き
		...
		input_data = numpy.stack(
			[temperature, rainfall, humidity, sea_level_pressure, 
			 wind_speed, wind_dir], 1)
...
def output_result_header():
	...
	fo.write('##################################################\n')
	fo.write('入力データ = 気温 降水量 相対湿度 海面気圧 風速 風向き\n')
	fo.write('model = 6 x 32 x 32 x 3\n')
...
if __name__ == '__main__':
	...
	for i in range(NUM_TRAING):
		...
		if (i % OUTPUT_CYCLE) == 0 :
			...
			acc_w = get_acc_by_weather(model, test_input, test_label, 1000)

...
NUMBER_OF_INPUT_NODES_FOR_REGRESS = 4 	# 入力データ数:(水戸,東京,静岡,大阪)x(降水量)
NUMBER_OF_HIDDEN_NODES_FOR_REGRESS = 32 # 隠れ層のノード数
NUMBER_OF_OUTPUT_NODES_FOR_REGRESS = 4	# 出力データ数:(水戸,東京,静岡,大阪)x(降水量)
DROPOUT_RATE_FOR_REGRESS = 0.1		# ドロップアウト率
LEARNING_RATE_FOR_REGRESS = 0.002	# 学習率
SIZE_OF_BATCH_FOR_REGRESS = 24		# バッチサイズ
RESULT_FILE_NAME_FOR_REGRESS  = './result3/result_190815_lstm_regress_06'
...
RESULT_FILE_WHOLE = './result3/result_190815_lstm_regress_06.csv'
...
def load_data_for_regress(dir_path, point_name):
	...
	re_input = numpy.stack([rainfall], 1)
	re_target = numpy.stack([rainfall], 1)
	#re_input = numpy.stack([temperature, rainfall, humidity, pressure], 1)
	#re_target = numpy.stack([temperature, rainfall, humidity, pressure], 1)
...
def get_data_for_regress():
	...
	train_input_raw5, train_target_raw5 = load_data_for_regress('./train/osaka',    '大阪')
	train_input_raw = numpy.hstack(
				[train_input_raw1, train_input_raw3, train_input_raw4, train_input_raw5] )
	train_target_raw = numpy.hstack(
				[train_target_raw1, train_target_raw3, train_target_raw4, train_input_raw5] )
	#train_input_raw = train_input_raw1.reshape(train_input_raw1.shape[0], NUMBER_OF_INPUT_NODES_FOR_REGRESS)
	...
	test_input_raw5, test_target_raw5 = load_data_for_regress('./test/osaka',    '大阪')
	test_input_raw = numpy.hstack(
				[test_input_raw1, test_input_raw3, test_input_raw4, test_input_raw5] )
	test_target_raw = numpy.hstack(
				[test_target_raw1, test_target_raw3, test_target_raw4, test_input_raw5] )
	#test_input_raw = test_input_raw1.reshape(test_input_raw1.shape[0], NUMBER_OF_INPUT_NODES_FOR_REGRESS)
...
def output_whole_result_header():
	...
	fo.write('##################################################\n')
	fo.write('入力データ = (水戸,東京,静岡,大阪)x(降水量)\n')
...
#def output_result_for_regress(input, target, predicted, scaler_input, scaler_target,number):
	...
	fo = open(filename, 'w')
	fo.write('水戸,東京,静岡,大阪,水戸(予測),,,,\n')
	fo.write('降水量,降水量,降水量,降水量,降水量,降水量,降水量,降水量,\n')
	...
	for i in range(data_len):
		...
		if i < sequence_len :
			...
			fo.write('%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f\n' %
				(input_i[0], input_i[1], input_i[2], input_i[3],
				 predicted[pi,0], predicted[pi,1], predicted[pi,2], predicted[pi,3], 
				) )

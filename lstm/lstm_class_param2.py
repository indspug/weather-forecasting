...
NUMBER_OF_INPUT_NODES_FOR_CLASS = 3  	# 入力データ数:(水戸)x(降水量,日射量,雲量)
...
RESULT_FILE_NAME_FOR_CLASS  = './result3/result_190816_lstm_class_02'
...
RESULT_FILE_WHOLE = './result3/result_190816_lstm_class_02.csv'
...
def load_data_for_class(dir_path, point_name):
	...
	daylight = numpy.array([])
	cloud_cover = numpy.array([])
	...
	for csv_path in csv_paths:
		...
		dayl = get_daylight(csv_data, point_name)
		daylight = numpy.append(daylight, dayl)
		clou = get_cloud_cover(csv_data, point_name)
		cloud_cover = numpy.append(cloud_cover, clou)
	...
	re_input = numpy.stack([rainfall, daylight, cloud_cover], 1)
...
def output_whole_result_header():
	...
	fo.write('##################################################\n')
	fo.write('入力データ = (水戸)x(降水量,日射量,雲量)\n')
...
def output_result_for_class(input, target, predicted, number):
	...
	fo.write('降水量,日射量,雲量,降水量,日射量,雲量,晴れ,曇り,雨,晴れ,曇り,雨,正解/不正解\n')
	...
	for i in range(data_len):
		...
		if i < sequence_len:
			...
			fo.write('%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,%d\n' %
				(input_i[0], input_i[1], input_i[2], 
